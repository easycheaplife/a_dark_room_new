...
```bash
# 🚀 简化测试入口（推荐）
dart run_tests.dart quick          # 快速测试套件（30秒，日常开发推荐）
dart run_tests.dart core           # 核心系统测试（2分钟，提交前验证）
dart run_tests.dart integration    # 集成测试（1分钟，功能开发完成后）
dart run_tests.dart all            # 所有测试（5分钟，发布前验证）
