...
void main() {
  group('护甲类物品button属性验证', () {
    ...
    test('验证所有护甲类物品都有button属性设置为null', () {
      Logger.info('🛡️ 开始验证护甲类物品button属性...');
      ...
      Logger.info('🎉 所有护甲类物品button属性验证通过！');
    });

    test('验证rifle武器也有正确的button属性', () {
      Logger.info('🔫 验证rifle武器button属性...');
      ...
      Logger.info('✅ rifle: button=${rifle['button']}, type=${rifle['type']}');
      Logger.info('🎉 rifle武器button属性验证通过！');
    });

    test('对比原游戏和Flutter项目的护甲配置一致性', () {
      Logger.info('🔍 对比护甲配置一致性...');
      ...
      for (final armorName in originalArmorConfigs.keys) {
        ...
        Logger.info('✅ $armorName 配置与原游戏完全一致');
      }

      Logger.info('🎉 所有护甲配置与原游戏保持一致！');
    });

    test('验证护甲类物品的解锁逻辑', () {
      Logger.info('🔓 验证护甲类物品解锁逻辑...');
      ...
      for (final armorName in armorItems) {
        ...
        Logger.info('✅ $armorName 解锁逻辑正常');
      }

      Logger.info('🎉 护甲类物品解锁逻辑验证通过！');
    });

    test('验证护甲类物品在UI中的显示逻辑', () {
      Logger.info('🖥️ 验证护甲类物品UI显示逻辑...');
      ...
      for (final armorName in armorItems) {
        ...
        Logger.info(
            '✅ $armorName: 需要工坊=$needsWorkshop, 本地化名称="$localizedName"');
      }

      Logger.info('🎉 护甲类物品UI显示逻辑验证通过！');
    });
  });
}
