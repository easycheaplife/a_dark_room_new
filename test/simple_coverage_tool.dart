...
void main() async {
  ...
  try {
    // 扫描源代码文件
    final sourceFiles = await scanSourceFiles();
    Logger.info('📁 发现源代码文件: ${sourceFiles.length} 个');

    // 扫描测试文件
    final testFiles = await scanTestFiles();
    Logger.info('🧪 发现测试文件: ${testFiles.length} 个');

    // 分析覆盖率
    final coverage = analyzeCoverage(sourceFiles, testFiles);

    // 生成报告
    await generateReport(coverage);

    Logger.info('✅ 测试覆盖率报告生成完成: docs/test_coverage_report.md');
  } catch (e) {
...
/// 扫描源代码文件
Future<List<String>> scanSourceFiles() async {
  final sourceFiles = <String>[];
  final libDir = Directory('lib');

  if (!await libDir.exists()) {
    throw Exception('源代码目录不存在: lib');
  }

  await for (final entity in libDir.list(recursive: true)) {
    if (entity is File && entity.path.endsWith('.dart')) {
      final relativePath =
          entity.path.replaceFirst('lib\\', '').replaceFirst('lib/', '');
...
/// 扫描测试文件
Future<List<String>> scanTestFiles() async {
  final testFiles = <String>[];
  final testDir = Directory('test');

  if (!await testDir.exists()) {
    throw Exception('测试目录不存在: test');
  }
  ...
  await for (final entity in testDir.list(recursive: true)) {
    if (entity is File &&
        ...
        !entity.path.endsWith('simple_coverage_tool.dart')) {
      final relativePath =
          entity.path.replaceFirst('test\\', '').replaceFirst('test/', '');
...
/// 分析测试覆盖率
Map<String, dynamic> analyzeCoverage(
  ...
  for (final file in sourceFiles) {
    ...
    if (parts.isEmpty) {
      final winParts = file.split('\\');
      final module = winParts.length > 1 ? winParts[0] : 'root';
  ...
  for (final module in moduleFiles.keys) {
    ...
    for (final file in files) {
      final fileName =
          file.split('/').last.split('\\').last.replaceAll('.dart', '');
      final hasTest = testFiles.any((test) =>
          test.contains(fileName) ||
          test.contains(module) ||
          isFileTestedByIntegrationTest(file, testFiles));
...
/// 检查文件是否被集成测试覆盖
bool isFileTestedByIntegrationTest(String file, List<String> testFiles) {
  final fileName =
      file.split('/').last.split('\\').last.replaceAll('.dart', '');
...
/// 生成报告文件
Future<void> generateReport(Map<String, dynamic> coverage) async {
  ...
  if (uncoveredFiles.isNotEmpty) {
    buffer.writeln('- 📝 **优先添加测试**: 为上述 ${uncoveredFiles.length} 个未覆盖文件添加测试');
  }
