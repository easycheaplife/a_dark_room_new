  ...
  group('🔧 GameConfig 配置项生效验证', () {
    ...
    group('🌍 World 模块配置验证', () {
      ...
      test('应该使用 GameConfig 中的治疗数值配置', () {
        ...
        Logger.info('✅ 治疗数值: 肉=${World.meatHeal}, 药=${World.medsHeal}, 注射器=${World.hypoHeal}');
      });
      ...
      test('应该使用 GameConfig 中的战斗相关配置', () {
        ...
        Logger.info('✅ 战斗配置: 命中率=${World.baseHitChance}, 战斗概率=${World.fightChance}, 延迟=${World.fightDelay}');
      });
      ...
      test('应该使用 GameConfig 中的移动消耗配置', () {
        ...
        Logger.info('✅ 移动消耗: 食物=${World.movesPerFood}步, 水=${World.movesPerWater}步, 死亡冷却=${World.deathCooldown}秒');
      });
    });
    ...
    group('🎒 Path 模块配置验证', () {
      test('应该使用 GameConfig.defaultBagSpace 配置', () {
        ...
        Logger.info('✅ Path.defaultBagSpace = ${Path.defaultBagSpace} (来自 GameConfig)');
      });
    ...
    group('🚀 Space 模块配置验证', () {
      test('应该使用 GameConfig 中的太空相关配置', () {
        ...
        Logger.info('✅ 太空配置: 飞船速度=${Space.shipSpeed}, 小行星延迟=${Space.baseAsteroidDelay}');
      });
      ...
      test('应该使用 GameConfig 中的星空配置', () {
        ...
        Logger.info('✅ 星空配置: 宽度=${Space.starWidth}, 高度=${Space.starHeight}, 星星数量=${Space.numStars}');
      });
    });
    ...
    group('🏠 Outside 模块配置验证', () {
      test('应该使用 GameConfig 中的外部模块配置', () {
        ...
        Logger.info('✅ 外部配置: 人口延迟=${GameConfig.popDelayRange}, 小屋容量=${GameConfig.hutRoom}');
        Logger.info('✅ 操作延迟: 伐木=${GameConfig.gatherWoodDelay}秒, 陷阱=${GameConfig.checkTrapsDelay}秒');
      });
    });
