...
### 🐛 Bug修复
- **铁矿访问修复** - 修复地图中铁矿访问后不会标记为已访问的问题
  - 问题：铁矿(I)访问后选择战斗，即便胜利后也不会变灰，还可以继续访问
  - 根本原因：铁矿在`_handleMissingSetpiece`中被立即标记为已访问，与setpiece场景逻辑冲突
  - 修复方案：移除立即标记逻辑，只在完成setpiece事件后才标记为已访问
  - 修复效果：
    - ✅ 铁矿访问后正确标记为已访问状态 (`I!`)
    - ✅ 与原游戏行为保持一致
    - ✅ 防止玩家重复访问获得奖励
  - 文档：`docs/05_bug_fixes/terrain_repeat_visit_fix.md`
